...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks {
    ...
    public static void syncWearable(final Context context, final GoogleApiClient googleApiClient) {
        String locationQuery = Utility.getPreferredLocation(context);
        Uri weatherUri = WeatherContract.WeatherEntry.buildWeatherLocationWithDate(locationQuery, System.currentTimeMillis());
        Cursor cursor = context.getContentResolver().query(weatherUri, NOTIFY_WEATHER_PROJECTION, null, null, null);
        if (cursor.moveToFirst()) {
            ...
            dataMap.putInt(WEATHER_ID_KEY, weatherId);
            dataMap.putString(HIGH_TEMP_KEY, high);
            dataMap.putString(LOW_TEMP_KEY, low);
            final PutDataRequest dataRequest = putDataMapRequest.asPutDataRequest().setUrgent();
    ...
    public void onConnected(@Nullable Bundle bundle) {
        syncWearable(getContext(), mGoogleApiClient);
    }
